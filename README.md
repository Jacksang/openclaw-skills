...
| **project-planner** | Stories → **API/UI split behavior tests** → UI/UX → architecture → **`UI_IMPLEMENTATION_MANIFEST.md`** (PAGE backlog) + **`UI_API_BINDING_MANIFEST.md`** (widget→endpoint map). | `uidesign/`, both manifests, `ARCHITECTURE.md`, … |
...
| **project-implementation** | **Dual-track:** backend (API tests) + **frontend (manifest PAGE tasks, UI gates N.3b/N.3c/N.3d)** → `test:all` + `test:ui` + `test:ui-bindings` locally before push. | Manifests on SCRUM_BOARD, `TEST_REPORT_E0X-UI.md` |
| **project-uat** | **Blocks if UI manifests incomplete or binding not verified** → UAT on local Docker → validation → AI + human rounds → sign-off. | `tests/uat/`, `UAT_SIGNOFF.md` |
